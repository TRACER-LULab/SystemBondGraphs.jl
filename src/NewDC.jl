function resolve_equations(
    constituitive_equation::Equation,
    derivative_form::Equation,
    algebraic_eqns::Vector{Equation},
    differential_eqns::Vector{Equation},
    ps,
    svs,
)
    ...
    check(x) = (
        isa(x, Term) &&
        (indexin([x], ps)[1] === nothing) &&
        (indexin([x], svs)[1] === nothing)
    )
    terms = filter(check, get_variables(constituitive_equation.rhs))
    terms_old = []
    while !check(constituitive_equation) &&
              length(intersect(terms, terms_old)) != length(terms) &&
              length(terms) != 0
        terms_old = terms
        for term in terms
            for index in eachindex(alg_eqs)
                ...
                if found
                    ...
                    lhs = term
                    constituitive_equation = flatten_fractions(
                        simplify(substitute(constituitive_equation, Dict(lhs => rhs))),
                    )
    ...
    constituitive_equation =
        constituitive_equation.lhs ~ flatten_fractions(simplify(constituitive_equation.rhs))
    for index in eachindex(differential_eqns)
        deq = differential_eqns[index]
        sub_dict = Dict(
            constituitive_equation.lhs =>
                flatten_fractions(simplify(constituitive_equation.rhs)),
        )
        ...
        derived_eqn_rhs =
            flatten_fractions(simplify(substitute(derived_eqn_rhs, dsub_dict)))
        differential_eqns[index] = deq.lhs ~ derived_eqn_rhs
        dsub = Dict(derivative_form.rhs => derivative_form.lhs)
        differential_eqns[index] =
            flatten_fractions(simplify(expand(substitute(differential_eqns[index], dsub))))
    end
...
function derivative_casuality(BG; skip = [])
    ...
    for node in dc_nodes
        ...
        filtered_eqs =
            [algebraic_eqns[1:(start_index-1)]; algebraic_eqns[(start_index+1):end]]
        # display("---------")
