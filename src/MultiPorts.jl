...
"""
function add_C_multiport!(
    BG::AbstractBondGraph,
    elements,
    parameters,
    name;
    ϕi = (e, q, params, BG) -> [],
    ...
    deriv_eqns =
        map(i -> D(𝐪[i]) ~ ParentScope(BG[elements[i].first].f), eachindex(elements))
    ...
    props = Dict(:type => :MPC, :sys => sys, :causality => false, :state_var => [])
