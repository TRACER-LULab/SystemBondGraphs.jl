...
function mapk_cascade_factory!(BG, name)
    ...
    add_0J!(
        BG,
        Dict(S(name * "MKKKK") => false, S(name * "cycle1_Kin") => true),
        S(name * "J_MKKKK"),
    )
    # Connect MKKK
    add_0J!(
        BG,
        Dict(S(name * "MKKK") => false, S(name * "cycle1_M") => true),
        S(name * "J_MKKK"),
    )
    # Connect MKKKPase
    add_0J!(
        BG,
        Dict(S(name * "MAPKKKPase") => false, S(name * "cycle1_Pho") => true),
        S(name * "J_MAPKKKPase"),
    )
    # Connect MKKKP
    add_0J!(
    ...
    add_0J!(
        BG,
        Dict(S(name * "MKK") => false, S(name * "cycle2_M") => true),
        S(name * "J_MKK"),
    )
    # Connect MKKP
    add_0J!(
    ...
    add_0J!(
        BG,
        Dict(S(name * "MK") => false, S(name * "cycle4_M") => true),
        S(name * "J_MK"),
    )
    # Connect MKP
    add_0J!(
    ...
    add_0J!(
        BG,
        Dict(S(name * "MKPP") => false, S(name * "cycle5_MP") => true),
        S(name * "J_MKPP"),
    )
    # Connect ATP
    add_0J!(
...
for index in eachindex(init_mkkkk)
    ...
    prob = remake(
        prob,
        u0 = ModelingToolkit.varmap_to_vars(u0, states(model)),
        tspan = (0.0, 1000.0),
    )
    sol = solve(prob)
    res[:, index] .=
        (sol[test[:MKKKP].q][end], sol[test[:MKKPP].q][end], sol[test[:MKPP].q][end])
end
...
plot(
    ...
    res',
    xscale = :log,
    labels = ["MKPP" "MKKPP" "MKKKP"],
    legend = :topright,
    xlabel = "Input (MKKKK) Concentration (μM)",
