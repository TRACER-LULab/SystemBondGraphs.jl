...
function swap_bond(BG, elem)
    ...
    add_Bond!(BG, :temp)
    swap(BG, elem, "temp")
    set_prop!(BG.graph, BG.graph[:temp, :name], :name, name)
end
## Kinase Factory
...
function mapk_cascade_factory!(BG, name)
    ...
    for c ∈ phosphorylation_cycles
        ...
        swap_bond(BG, S(name*c[:c]*"Kin"))
        swap_bond(BG, S(name*c[:c]*"Pho"))
        swap_bond(BG, S(name*c[:c]*"M"))
        swap_bond(BG, S(name*c[:c]*"MP"))
        swap_bond(BG, S(name*c[:c]*"ATP"))
        swap_bond(BG, S(name*c[:c]*"ADP"))
        swap_bond(BG, S(name*c[:c]*"P"))
    ...
    for s ∈ species
        display(species_dict[s])
        add_0J!(BG, species_dict[s], S(name*"J_"*s))
    end
    for c ∈ chemostats 
        display(chemostat_dict[c])
